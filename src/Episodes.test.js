...
//gets episodes prop from App = the data 
...
test('Episodes component shows data when rerendered with new episodes data', () => {
    const { queryAllByTestId, rerender } = render(
        <Episodes episodes={[]} />
      );

    expect(queryAllByTestId('episodesid')).toStrictEqual([]);
    expect(queryAllByTestId('episodesid')).toHaveLength(0);
